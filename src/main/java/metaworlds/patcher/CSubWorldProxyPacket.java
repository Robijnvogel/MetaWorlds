...
public class CSubWorldProxyPacket extends MetaWorldsPacket {
    ...
    public void read(ChannelHandlerContext ctx, ByteBuf buf)
    {
        ...
        if (fmlProxyPacket.getTarget().isClient())
            return;//this.networkManager = ((NetHandlerPlayClient)fmlProxyPacket.handler()).getNetworkManager();
        else
            this.networkManager = ((NetHandlerPlayServer)fmlProxyPacket.handler()).netManager;
